...
run-entrypoint:
	docker run --entrypoint /bin/bash -v ${PWD}/sample/db.properties:/etc/backup/db.properties -v ${PWD}/sample/restic.properties:/etc/backup/restic.properties -v ${PWD}/sample/dbTest.sh:/scripts/dbTest.sh -it  --rm opstree/mysqlbackup:0.1 

put-dummy-data:
	docker run -it --entrypoint /bin/bash --rm  opstree/mysqlbackup:0.1 -c "source /scripts/dbTest.sh writeToDatabase"
